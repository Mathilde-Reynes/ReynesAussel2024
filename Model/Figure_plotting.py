...
def figure_plotting(fig_number, all_monitors, all_monitors_T, all_synapses, monitor_poisson, runtime, N):
    ...
    if fig_number == "5" :
        ...
        im2 = ax2.imshow(V2_TC.v/mV, aspect='auto', cmap='YlGnBu', vmax=-60, vmin=-75, 
    ...
    if fig_number == "14":
        ...
        fig.tight_layout()
        #
        ax[4].plot(I1_RE.t/ms, I1_RE.IsynAMPA_PY_RE[n_choice]/(0.01*amp*meter**-2),color="black",linewidth=1.5)
        ax[4].spines['top'].set_visible(False)
        ax[4].spines['right'].set_visible(False)
        ax[4].spines['bottom'].set_visible(True)
        ax[4].spines['left'].set_visible(True)
    ...
    # Figure 16 & S3
    if fig_number in ["16-A1", "16-A2", "16-A3", "16-A4", "16-B1", "16-B2", "16-B3", "16-B4", "16-B5", "16-B6", "16-B7","S3"]:
        fig, ax = plt.subplots(figsize=(15, 5))
        ax.plot(V2_PYs.t, V2_PYs.v[N//2]/mV, color="tab:blue", linewidth=1.5)
    ...
    if fig_number in ["17-A1","18-A1","17-A2","18-A2","17-A3","18-A3","17-B1","18-B1","17-B2","18-B2","17-B3","18-B3"]:
        ...
        if fig_number in ["18-A1","18-A2","18-A3","18-B1","18-B2","18-B3"]:
            ...
            rangey = [
                min(crosscorr[int(len(crosscorr)//2 - 5*rhs_sampling_freq):
                                        int(len(crosscorr)//2 + 5*rhs_sampling_freq)]),
                max(crosscorr[int(len(crosscorr)//2 - 5*rhs_sampling_freq):
                                        int(len(crosscorr)//2 + 5*rhs_sampling_freq)])
            ]
            ...
            plt.subplot(4, 1, 2) 
            plot_without_fill(plt.gca(), freqs, Spectrum_PY, 'Frequency (Hz)', 'PY', '', show_xlabel=False, x_major_locator_base=1)
            plt.ylabel('PY', fontsize=30)
            plt.subplot(4, 1, 3, sharex=plt.gca()) 
            plot_without_fill(plt.gca(), freqs, Spectrum_TC, 'Frequency (Hz)', 'TC', '', show_xlabel=False, x_major_locator_base=1)
            plt.subplot(4, 1, 4, sharex=plt.gca()) 
            plot_without_fill(plt.gca(), freqs, Spectrum_input, 'Frequency (Hz)', 'Input', '', show_xlabel=True, x_major_locator_base=1)
            plt.xlim(0, 4)
            plt.tight_layout()
            plt.show()
