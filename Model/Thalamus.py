...
def create_thalamic_subparts(N):
    ...
    RE = NeuronGroup(
        ...
        refractory=3 * ms,
        events={
            "custom_poisson_PY": "v>1000*mV",
            "custom_poisson_IN": "v>1000*mV",
            "custom_poisson_inter": "v>1000*mV",
        },
    )
    RE.v = -61 * mV
    RE.m_na = 0.01
    ...
    S_GABAA_RE_RE = syn_gabaa_thal(
        RE,
        RE,
        "IsynGABAA_RE_RE",
        s_RE,
        "abs(i-j)<=" + str(RE_RE) + " and i!=j",
        g_syn_gabaa_rere,
        E_gabaa,
        alpha_gabaa_thal,
        beta_gabaa_thal,
    ...
    S_AMPA_TC_RE = syn_ampa_thal(
        TC, RE, "IsynAMPA_TC_RE", s_RE, "abs(i-j)<=" + str(TC_RE) + "", g_syn_ampa_tcre
    )
    ...
    I2 = StateMonitor(
        TC, ("IsynGABAB_RE_TC", "IsynGABAA_RE_TC", "I_h", "I_t", "ratio", "E_ca_Thal"), record=True
    )
    # Synapses monitoring
