        ...
        if (raster_PY.t[k] - raster_PY.t[k - 1]) > min_silence or list_up_states_beginning == []:
            ...
            try:  # if the simulations ends in an up-state there can be bugs
                ...
                last_neuron_spiking = argmax(
                    [raster_PY.t[k + argwhere(raster_PY.i[k:] == N)[0]] for N in range(100)]
                ...
                print(
                    "The last neuron to spike in this up-state is neuron N="
                    + str(last_neuron_spiking)
                    + " at time t="
                    + str(time_last_neuron_spiking)
                )
...
def thalamocortical_network(seed_value, analyze_speed, fig_number, raw_data, plot_figure):
    ...
    print(
        "Updating N and minis amplitudes before instantiating Thalamus & Cortical later for Figure n°: "
        + str(fig_number)
    )
    ...
    # Layer Cortex
    if fig_number == "11-1":
        all_neurons, all_synapses, all_gap_junctions, all_monitors = create_cortical_layer(
            N, log, A_PY_PY, A_PY_IN
        )
    else:
        all_neurons, all_synapses, all_gap_junctions, all_monitors = create_cortical_layer(
            N, exp, A_PY_PY, A_PY_IN
        )
    ...
    if fig_number in [
        ...
        Poisson_stim = PoissonGroup(
            N_TC // 4, rates="base_rate + 0.9 * base_rate * sin(2 * pi * modulation * t)"
        )
        S_AMPA_stim_TC = syn_ampa_thal(
            Poisson_stim, TC, "IsynAMPA_stim_TC", s_TC, "j > 18 and j < 31", g_syn_ampa_stim
        )
    ...
    if plot_figure:
        figure_plotting(
            str(fig_number), all_monitors, all_monitors_T, all_synapses, monitor_poisson, runtime, N
        )
