...
def syn_gabaa(source, target, syntype, surfacetarget, connection_pattern, g_syn, alpha, beta):
    ...
    S = Synapses(
        source,
        target,
        model=syntype + eq_syn_GABAA,
        method="rk4",
        on_pre={"pre": pre_code_gabaa, "mini_gabaa": poisson_gabaa},
        on_event={"mini_gabaa": "custom_poisson_gabaa"},
    )
...
def syn_ampa(
    ...
    S = Synapses(
        source,
        target,
        model=(syntype + eq_syn_AMPA) + (poissontype + eq_syn_AMPA2),
        method="rk4",
        on_pre={"pre": pre_code_ampa, "mini_PY": poisson_PY, "mini_IN": poisson_IN},
        on_event={"mini_PY": "custom_poisson_PY", "mini_IN": "custom_poisson_IN"},
    )
    if connection_pattern == "":
        S.connect()
    else:
