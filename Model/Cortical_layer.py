...
def create_cortical_layer(N, mini_form, A_PY_PY, A_PY_IN):
    ...
    if mini_form == "exp":
        PY_soma = NeuronGroup(
            ...
            refractory=3 * ms,
            events={
                "custom_poisson_PY": "rand()<mean_rate_PY*dt",
                "custom_poisson_IN": "rand()<mean_rate_IN*dt",
            },
        )
    ...
    if mini_form == "exp":
        IN_soma = NeuronGroup(
            ...
            threshold="v>20*mV",
            refractory=3 * ms,
            events={"custom_poisson_gabaa": "rand()<mean_rate_GABAA*dt"},
        )
    else:  # in case other value, take the standard log minis
        IN_soma = NeuronGroup(
            N_IN,
    ...
    # AMPA
    S_AMPA_PY_PY = syn_ampa(
        PY_soma,
        PY_dendrite,
        "IsynAMPA_PY_PY",
        s_Dend_PY,
        "abs(i-j)<=" + str(PY_PY) + " and i!=j",
        g_syn_ampa_pypy,
        A_PY_PY,
        "IEPSPs_PY_PY",
        0,
    ...
    S_AMPA_PY_IN = syn_ampa(
        PY_soma,
        IN_dendrite,
        "IsynAMPA_PY_IN",
        s_Dend_IN,
        "abs(floor(i*" + str(N_IN) + "/" + str(N_PY) + ") -j)<=" + str(PY_IN) + "",
        g_syn_ampa_pyin,
        A_PY_IN,
        "IEPSPs_PY_IN",
        1,
    ...
    # GABAA
    S_GABAA_IN_PY = syn_gabaa(
        IN_soma,
        PY_dendrite,
        "IsynGABAA_IN_PY",
        s_Dend_PY,
        "abs(floor(i*" + str(N_PY) + "/" + str(N_IN) + ") -j)<=" + str(IN_PY) + "",
        g_syn_gabaa_inpy,
        alpha_gabaa,
        beta_gabaa,
    )
