...
v_bazhenov=genfromtxt('../../Data_bazhenov/v_SOMA_IN')
Ik_bazhenov=genfromtxt('../../Data_bazhenov/I_k_IN')[:,15]
Ina_bazhenov=genfromtxt('../../Data_bazhenov/I_na_IN_soma')[:,15]
hna_bazhenov=genfromtxt('../../Data_bazhenov/h_na_IN')[:,15]
mna_bazhenov=genfromtxt('../../Data_bazhenov/m_na_IN')[:,15]
v_Ena=genfromtxt('../../Data_bazhenov/v_m_E_Na_in')[:,15]
...
ax[0].plot(time_bazhenov, v_bazhenov[:int((simulation_time / 0.02) / ms), 15], color="black", linewidth=1.3)
...
ax[1].plot(time_bazhenov, Ina_bazhenov[:int((simulation_time / 0.02) / ms)] / 1000, label=b, color="#d0db61", alpha=alphaa, linewidth=1.3)
configure_axis(ax[1], 'Fast sodium current, $I_{Na}$', 'mA/cm²', 10, 0, 1000, show_legend=True)
#
ax[2].plot(V1.t / ms, V1.I_k[0] / (10 * amp * meter**-2), label=a, color="#4B9CD3", linewidth=linea)
ax[2].plot(time_bazhenov, Ik_bazhenov[:int((simulation_time / 0.02) / ms)] / 1000, label=b, color="#d0db61", linewidth=1.3, alpha=alphaa)
...
fig, ax = plt.subplots(6, 1, sharex=True, figsize=(15, 25))
...
#
ax[1].plot(V1.t / ms, v_Ena_us/mV, label=a, color="#4B9CD3", linewidth=linea)
ax[1].plot(time_bazhenov, v_Ena[:int((simulation_time / 0.02) / ms)], label=b, color="#d0db61", alpha=alphaa, linewidth=lineb)
configure_axis(ax[1], 'Fast sodium electrochemical driving force (v-E)', 'mV', 25, 414.25, 415.5, show_legend=True)
#
ax[2].plot(V1.t / ms, V1.I_na[0] / (10 * amp * meter**-2), label=a, color="#4B9CD3", linewidth=linea)
ax[2].plot(time_bazhenov, Ina_bazhenov[:int((simulation_time / 0.02) / ms)] / 1000, label=b, color="#d0db61", alpha=alphaa, linewidth=lineb)
configure_axis(ax[2], 'Fast sodium current, $I_{Na}$', 'mA/cm²', 10, 414.25, 415.5, show_legend=True)
#
ax[3].plot(V1.t / ms, V1.m_na[0], label=a, color="#4B9CD3", linewidth=linea)
ax[3].plot(time_bazhenov, mna_bazhenov[:int((simulation_time / 0.02) / ms)], label=b, color="#d0db61", linewidth=lineb, alpha=alphaa)
configure_axis(ax[3], 'Activation gating variable $m$ for $I_{Na}$', '', 0.5, 414.25, 415.5, show_legend=True)
#
ax[4].plot(V1.t / ms, V1.h_na[0], label=a, color="#4B9CD3", linewidth=linea)
ax[4].plot(time_bazhenov, hna_bazhenov[:int((simulation_time / 0.02) / ms)], label=b, color="#d0db61", linewidth=lineb, alpha=alphaa)
configure_axis(ax[4], 'Inactivation gating variable $h$ for $I_{Na}$', '', 0.5, 414.25, 415.5, show_legend=True)
#
ax[5].plot(V1.t / ms, V1.I_k[0] / (10 * amp * meter**-2), label=a, color="#4B9CD3", linewidth=linea)
ax[5].plot(time_bazhenov, Ik_bazhenov[:int((simulation_time / 0.02) / ms)] / 1000, label=b, color="#d0db61", linewidth=lineb, alpha=alphaa)
configure_axis(ax[5], 'Fast potassium current, $I_K$', 'mA/cm²', 10, 414.25, 415.5, show_legend=True)
ax[5].set_xlabel('Time (ms)', size=30, labelpad=25)
#
