...
g_syn_gabaa_retc = 0.0002*msiemens
...
#
ax[2].plot(V_TC.t/ms,V_TC.IsynAMPA_PY_TC[0]/ (0.1 * amp * meter**-2), label=a, color="#2A52BE", linewidth=linea)
ax[2].plot(time_bazhenov,a_CX_TC_bazhenov[:int((simulation_time/0.02)/ms)], label=b, color="#4B9CD3", linewidth=lineb, alpha=alphaa)
configure_axis(ax[2], 'Fast potassium current, $I_{K}$', 'mA/cm²', 0.02, 0, 1000, show_legend=True)
...
fig, ax = plt.subplots(3, 1, sharex=True, figsize=(15, 15))
ax[0].plot(time_bazhenov,v_PY_soma_bazhenov[:int((simulation_time/0.02)/ms),50], color="black", linewidth=lineb)
configure_axis(ax[0], 'Membrane potential used for computations', 'mV', 25, 420, 430)
#
ax[1].plot(time_bazhenov,v_TC_bazhenov[:int((simulation_time/0.02)/ms),25], color="black", linewidth=lineb)
configure_axis(ax[1], 'Fast sodium current, $I_{Na}$', 'mA/cm²', 10, 420, 430, show_legend=True)
#
#ax[2].plot(V_TC.t/ms,V_TC.IsynAMPA_PY_TC[0]/(10*amp*meter**-2), label=a, color="#2A52BE", linewidth=linea)
ax[2].plot(time_bazhenov,a_CX_TC_bazhenov[:int((simulation_time/0.02)/ms)], label=b, color="#4B9CD3", linewidth=lineb, alpha=alphaa)
configure_axis(ax[2], 'Fast potassium current, $I_{K}$', 'mA/cm²', 0.02, 420, 430, show_legend=True)
ax[2].set_xlabel('Time (ms)', size=30, labelpad=25)
#
#plt.savefig('Figure3A.png', dpi=300, bbox_inches='tight')
plt.show()